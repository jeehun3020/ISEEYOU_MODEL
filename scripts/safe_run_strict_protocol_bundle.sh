...
master_log="logs/strict_protocol_bundle_${timestamp}.log"
...
run_step strict_protocol_audit \
  python scripts/audit_split_protocol.py \
  --config configs/baseline_binary_allvideo_textmask_protocol.yaml

run_step strict_protocol_metadata_dummy \
  python scripts/metadata_dummy_baseline.py \
  --config configs/baseline_binary_allvideo_textmask_protocol.yaml \
  --cache-csv outputs/eval/metadata_dummy_cache_allvideo_textmask_protocol.csv

run_step strict_protocol_eval_pretrained \
  python eval.py \
  --config configs/baseline_binary_allvideo_textmask_protocol.yaml \
  --checkpoint outputs/checkpoints_allvideo_binary_textmask/best.pt \
  --split test

run_step strict_protocol_operating_pretrained \
  python scripts/eval_lowfpr_calibration.py \
  --config configs/baseline_binary_allvideo_textmask_protocol.yaml \
  --checkpoint outputs/checkpoints_allvideo_binary_textmask/best.pt \
  --split test
